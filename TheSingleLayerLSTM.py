...
class TextLSTM(nn.Module):
    ...
    def forward(self, X):
        ...
        for t in range(step):
            ...
            # i_t = σ(W_ii @ x_t + b_ii + W_hi @ h_t-1 + b_hi)
            # f_t = σ(W_if @ x_t + b_if + W_hf @ h_t-1 + b_hf) 
            # g_t = tanh(W_ig @ x_t + b_ig + W_hg @ h_t-1 + b_hg)
            # o_t = σ(W_io @ x_t + b_io + W_ho @ h_t-1 + b_ho)
